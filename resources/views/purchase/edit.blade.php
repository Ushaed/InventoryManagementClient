...
@section('title-card-small','Edit Purchase')
@section('timelineBar')
...
{{--    @if($data['purchases']['status'] ==1)--}}
    ...
    <div class="container fullbody">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('purchases.update',$data['purchases']['id']) }}">
                        @csrf
                        @method('PUT')
                        <div class="form-row">
                            <div class="form-group col-sm-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="supplier_id" id="supplier_id" class="form-control select2" required @if($data['purchases']['status'] ==2) disabled @endif>
                                        <option value="" selected>Select Supplier</option>
                                        @foreach($data['supplier'] as $supplier)
                                            <option
                                                value="{{ $supplier['id'] }}" @if ($data['purchases']['supplier_id'] == $supplier['id']) {{ 'selected' }} @endif>{{ $supplier['name'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-sm-8">
                                ...
                                <input type="text" id="product_name" name="product_name" class="typeahead form-control"
                                       placeholder="Product Name" autocomplete="off" @if($data['purchases']['status'] ==2) disabled @endif>
                        ...
                        <table class="table table-striped table-bordered table-sm" id="product_info_table">
                            ...
                            <tbody id="addRow">
                            @foreach($data['purchase_products'] as $key => $value)
                                <tr id="row_{{$loop->iteration}}">
                                    <td>
                                        <input type="hidden" class="form-control" name="purchase_product_id[]" id="puchase_product_id_{{$loop->iteration}}" value="{{$value['product_id']}}">{{ $value['product']['name'] }}</td>
                                    <td style='width: 13%'><input type='number' class='form-control' name='purchase_quantity[]' id="purchase_quantity_{{$loop->iteration}}" value="{{$value['quantity']}}" min='1' onkeyup='getSubTotal("{{$loop->iteration}}")' autocomplete='off' @if($data['purchases']['status'] == 2) disabled @endif></td>
                                    <td style='width: 13%'><input type='number' class='form-control' name = 'purchase_price[]' id='purchase_price_{{$loop->iteration}}' value="{{$value['price']}}" min='1' onkeyup='getSubTotal({{$loop->iteration}})' autocomplete='off' @if($data['purchases']['status'] == 2) disabled @endif></td>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control textarea" id="remarks"
                                              ...
                                              style="width: 100%; min-height: 149px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{$data['purchases']['remarks']}}</textarea>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-6">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" id="gross_amount"
                                                       ...
                                                       value="{{$data['purchases']['gross_total']}}">
                                                <input type="hidden" class="form-control" id="gross_amount_value"
                                                       name="gross_amount_value" autocomplete="off" value="{{$data['purchases']['gross_total']}}">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-7">
                                                <input type="number" class="form-control" id="discount" name="discount"
                                                       ...
                                                       value="{{$data['purchases']['discount']}}" @if($data['purchases']['status'] == 2) disabled @endif>
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-7">
                                                <input type="text" class="form-control" id="net_amount"
                                                       ...
                                                       value="{{$data['purchases']['net_total']}}">
                                                <input type="hidden" class="form-control" id="net_amount_value"
                                                       name="net_amount_value" autocomplete="off"  value="{{$data['purchases']['net_total']}}">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label for="net_amount" class="col-sm-5 control-label">Status</label>
                                            <div class="col-sm-7">
                                                <select name="status" id="status" class="form-control" @if($data['purchases']['status'] == 2) disabled @endif>
                                                    <option value="1" @if($data['purchases']['status'] == 1) selected @endif>Unapproved</option>
                                                    <option value="2" @if($data['purchases']['status'] == 2) selected @endif>Approved</option>
                                                </select>
                                            </div>
                                        </div>
                                        @if($data['purchases']['status'] == 1)
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.error === true) {
                            // alert("Something happning not correct!")
